	...
	return `
		<div class="list-item" data-item-index="${itemIndex}" >
			...
			<div class="radio-toolbar">
			    <input type="radio" name="question-${index}-answers" id="question-${index}-answers-A" value="A" ${item.chosenA ? 'checked' : ''} />
			    <label for="question-${index}-answers-A">A. ${item.optionA}<span class='correct-answer'></span></label>
			</div>
			<br>
			<div class="radio-toolbar">
			    <input type="radio" name="question-${index}-answers" id="question-${index}-answers-B" value="B" ${item.chosenB ? 'checked' : ''} />
			    <label for="question-${index}-answers-B">B. ${item.optionB}<span class='correct-answer'></span></label>
			</div>
			<br>
			<div class="radio-toolbar">
			    <input type="radio" name="question-${index}-answers" id="question-${index}-answers-C" value="C" ${item.chosenC ? 'checked' : ''} />
			    <label for="question-${index}-answers-C">C. ${item.optionC}<span class='correct-answer'></span></label>
			</div>
			<br>
			<div class="radio-toolbar">
			    <input type="radio" name="question-${index}-answers" id="question-${index}-answers-D" value="D" ${item.chosenD ? 'checked' : ''} />
...
function chooseAnswer() {
	$('.js-quiz-list').on('click', 'input', event => {
		...
		// use an if statement to change the 'checked' radio, though we already change the database, we need to change the rendered option
		if($input.attr('value') === 'A'){
			STORE[index].chosenA = true;
			STORE[index].chosenB = false;
			STORE[index].chosenC = false;
			STORE[index].chosenD = false;					
		} else if($input.attr('value') === 'B'){
			STORE[index].chosenA = false;
			STORE[index].chosenB = true;
			STORE[index].chosenC = false;
			STORE[index].chosenD = false;					
		} else if($input.attr('value') === 'C'){
			STORE[index].chosenA = false;
			STORE[index].chosenB = false;
			STORE[index].chosenC = true;
			STORE[index].chosenD = false;		
		} else {
			STORE[index].chosenA = false;
			STORE[index].chosenB = false;
			STORE[index].chosenC = false;
			STORE[index].chosenD = true;					
		}
		// let $siblings = $self.closest('div').siblings();
